...
def extract_tokens_options(func):
    ...
    @click.option('-m', '--pos-paddings', default='', help='POS tags to be padded.', type=click.STRING)
    ...
    @click.option('--tf-threshold', default=1, type=click.IntRange(1, 99), help='Global TF min threshold')
    @click.option('--tf-threshold-mask', default=False, is_flag=True, help='Mask low TF tokens as "__low_tf__"')
...
def tokens_transform_options(func):
    @click.option(
        '-r',
        '--remove-stopwords',
        default=None,
    ...
    @click.option('--only-alphabetic', default=False, is_flag=True, help='Keep only all-alphabetic tokens')
    @click.option(
