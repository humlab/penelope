...
def load_document_index(filename: str, key_column='document_id') -> pd.DataFrame:
    index = pd.read_csv(filename, sep='\t', index_col=0)
    if 'document_id' not in index.columns:
        if key_column != 'document_id' and key_column in index.columns:
            index['document_id'] = index[key_column]
        else:
            index['document_id'] = index.index
    return index
