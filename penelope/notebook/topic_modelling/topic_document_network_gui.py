...
def plot_document_topic_network(
    ...
    document_source: ColumnDataSource = network_utility.get_node_subset_source(network, layout_data, document_nodes)
    topic_source: ColumnDataSource = network_utility.get_node_subset_source(
        network, layout_data, topic_nodes, color_map=color_map
    )
    lines_source: ColumnDataSource = network_utility.get_edges_source(network, layout_data, scale=6.0, normalize=False)
