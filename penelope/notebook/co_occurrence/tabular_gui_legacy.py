...
class CoOccurrenceTable1(GridBox):  # pylint: disable=too-many-ancestors
    def __init__(
        self,
        *,
        bundle: Bundle,
        ...
        **kwargs,
        ...
        if not isinstance(bundle.co_occurrences, (dict, pd.DataFrame)):
            raise ValueError(f"Expected dict or DataFrame, found {type(bundle.co_occurrences)}")
        ...
        self.co_occurrences: pd.DataFrame = (
            pd.DataFrame(bundle.co_occurrences) if isinstance(bundle.co_occurrences, dict) else bundle.co_occurrences
        )

        self.token2id: pd.DataFrame = bundle.token2id
        self.document_index: pd.DataFrame = bundle.document_index
        self.concepts: Set[str] = bundle.context_opts.concept if bundle.context_opts else set()
        ...
        # self._filter.on_click(self.update_co_occurrences)
        self._save.on_click(self.save)
        self._download.on_click(self.download)
        self._global_threshold_filter.observe(self.update_co_occurrences, 'value')
        self._record_count_limit.observe(self.update_co_occurrences, 'value')
        self._token_filter.observe(self.update_co_occurrences, 'value')
    ...
    def update_co_occurrences(self, *_):
        data = self.get_data()
