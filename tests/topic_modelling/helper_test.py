...
def test_weights_reducer(state: TopicModelContainer):
    ...
    reducer: helper.DocumentTopicsCalculator = helper.DocumentTopicsCalculator(state.inferred_topics)
    ...
    assert reducer.reset().filter_by_keys(year=2019).value.year.unique().tolist() == [2019]
    assert reducer.reset().filter_by_keys(year=[2019]).value.year.unique().tolist() == [2019]
    assert reducer.reset().filter_by_keys(year=[0]).value.year.unique().tolist() == []
    assert len(reducer.reset().filter_by_keys(year=[2019], document_id=[0, 1]).value) == 8
    ...
    assert reducer.reset().filter_by_data_keys(year=[2019]).value.year.unique().tolist() == [2019]
    assert len(reducer.reset().filter_by_data_keys(document_id=0).value) == 4
    assert len(reducer.reset().filter_by_data_keys(topic_id=0).value) == 5
    ...
    data: pd.DataFrame = reducer.reset().filter_by_document_keys(title='Nocturne').value
    assert len(data) == 4
    assert data.document_id.unique().tolist() == [1]

    data: pd.DataFrame = reducer.reset().filter_by_document_keys(n_tokens=(operator.ge, 100)).value
    assert data.document_id.unique().tolist() == [2]

    data: pd.DataFrame = reducer.reset().filter_by_document_keys(n_tokens=(operator.lt, 60)).value
    ...
    data: pd.DataFrame = (
        reducer.reset().threshold(threshold=0.1).filter_by_keys(year=[2019, 2020], n_tokens=('ge', 60)).value
    )
